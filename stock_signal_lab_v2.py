...
_cool_theme = {
    "primaryColor": "#FF72B6", "backgroundColor": "#FFF7FC",
    "secondaryBackgroundColor": "#FFE3F1", "textColor": "#4A2740",
    "borderColor": "#E7A7C7", "dataframeBorderColor": "#E7A7C7",
    "dataframeHeaderBackgroundColor": "#FFD4E8", "codeBackgroundColor": "#FFEAF5",
    "linkColor": "#FF5CA8", "codeTextColor": "#7A4762",
    "chartCategoricalColors": ["#FF72B6", "#C995FF", "#FFAACF", "#FF9FE0"],
    "chartSequentialColors": ["#FFF7FC", "#FFEAF5", "#FFD4E8", "#FFAACF", "#FF72B6", "#C995FF"],
    "chartDivergingColors": ["#C995FF", "#FF9FE0", "#FFF7FC", "#FFAACF", "#FF72B6"],
}
for _hue in ("red", "orange", "yellow", "blue", "green", "violet", "gray"):
    _cool_theme.update({_hue + "Color": "#FF72B6", _hue + "BackgroundColor": "#FFE3F1",
                        _hue + "TextColor": "#7A4762"})
...
st.markdown("""
<style>
:root { color-scheme: light; --ink:#4a2740; --secondary:#7a4762; --muted:#a25f82; --border:#4a2740; --subtle:#ffe3f1; --surface:#fff7fc; }
.stApp { background:linear-gradient(145deg,#fff7fc 0%,#ffeaf5 45%,#f7ddff 100%) fixed; color:var(--ink); }
...
[data-testid="stSidebar"] { background:linear-gradient(180deg,#fff1f9 0%,#ffdced 100%); border-right:2px solid var(--ink); }
[data-testid="stSidebar"] .block-container { padding-top:2.2rem; }
.block-container { max-width:1080px; padding-top:1.15rem; padding-bottom:5rem; }
...
.app-hero { background:linear-gradient(125deg,#ffd9ec 0%,#ffeaf7 52%,#f3d7ff 100%); padding:2rem 2.1rem 1.8rem; margin:0 9px 2.8rem 0; border:3px solid var(--ink); box-shadow:8px 8px 0 var(--ink); }
...
[data-testid="stMetric"] { background:linear-gradient(135deg,#fff9fd,#ffe6f4); border:2px solid var(--ink); border-radius:0; box-shadow:4px 4px 0 var(--ink); padding:1rem 1.1rem; min-height:108px; margin:0 4px 5px 0; }
...
.stButton button,.stDownloadButton button,[data-testid="stFormSubmitButton"] button { background:linear-gradient(135deg,#ffd9ec,#f5d9ff); color:var(--ink)!important; border:2px solid var(--ink); border-radius:0; font-size:.84rem; font-weight:800; letter-spacing:.005em; min-height:43px; box-shadow:3px 3px 0 var(--ink); transition:background .14s ease,transform .14s ease,box-shadow .14s ease; }
.stButton button:hover,.stDownloadButton button:hover,[data-testid="stFormSubmitButton"] button:hover { background:linear-gradient(135deg,#ffc2df,#efc2ff); border-color:var(--ink); color:var(--ink)!important; transform:translate(-2px,-2px); box-shadow:5px 5px 0 var(--ink);  }
...
.stButton button[kind="primary"], [data-testid="stFormSubmitButton"] button[kind="primary"] { background:#ff72b6; border-color:var(--ink); color:#fff!important; }
.stButton button[kind="primary"]:hover,[data-testid="stFormSubmitButton"] button[kind="primary"]:hover { background:#ff5ca8; border-color:var(--ink); color:#fff!important; }
...
[data-testid="stSidebar"] .stButton button:hover { background:#ffd8ea; border-color:var(--ink); transform:none; box-shadow:none; }
[data-baseweb="input"], [data-baseweb="select"]>div,[data-baseweb="textarea"] { background:#fffafd!important; border:2px solid var(--ink)!important; border-radius:0!important; color:var(--ink)!important; }
[data-testid="stTextInput"] [data-baseweb="base-input"],
[data-testid="stTextInput"] [data-baseweb="input"]>div,
[data-testid="stTextInput"] input,
[data-testid="stNumberInput"] input,
[data-baseweb="select"] input { background:#fafcfd!important; background-image:none!important; color:#4a2740!important; -webkit-text-fill-color:#4a2740!important; caret-color:#ff72b6!important; border-radius:0!important; }
[data-testid="stTextInput"] input::placeholder { color:#a25f82!important; -webkit-text-fill-color:#a25f82!important; opacity:1; }
[data-testid="stTextInput"] input:autofill { box-shadow:0 0 0 1000px #fafcfd inset!important; }
[data-testid="stTextInput"] input:-webkit-autofill { -webkit-box-shadow:0 0 0 1000px #fafcfd inset!important; -webkit-text-fill-color:#4a2740!important; }
...
[data-testid="stExpander"] { background:#fffafd; border:2px solid var(--ink); border-radius:0; overflow:hidden; }
[data-baseweb="tab-list"] { gap:1.3rem; border-bottom:2px solid var(--ink); padding:0; margin:1.4rem 0 1.1rem; }
button[data-baseweb="tab"] { color:#a25f82; border-radius:0; background:none; font-weight:650; padding:.8rem .15rem; transition:color .17s ease; }
...
.signal-pill { color:var(--ink); background:#ffd9ec; border:2px solid var(--ink); border-radius:0; font-size:.75rem; font-weight:750; padding:8px 12px; white-space:nowrap; }
.empty-state { background:linear-gradient(135deg,#fff9fd,#ffe6f4); border:2px solid var(--ink); box-shadow:4px 4px 0 var(--ink); border-radius:0; padding:1.3rem 1.5rem; margin:1rem 0 2rem; }
...
[data-testid="stAlert"] { background:#ffe8f4!important; border:2px solid var(--ink)!important; border-radius:0; color:var(--ink)!important; }
[data-testid="stAlert"] svg { color:#354c60!important; }
[data-testid="stVegaLiteChart"] { box-sizing:border-box; width:100%; min-width:0; padding:0!important; margin:14px 0 24px; background:#fffafd; border:0; box-shadow:inset 0 0 0 2px var(--ink); }
[data-baseweb="tag"] { background:#ffd9ec!important; color:#4a2740!important; border-radius:0!important; }
[data-baseweb="tag"] span,[data-baseweb="tag"] svg { color:#4a2740!important; }
[data-baseweb="slider"] [role="slider"] { background:#ff72b6!important; border-color:#ff72b6!important; }
[data-baseweb="slider"] div:has(>[role="slider"]) { background:#ff72b6!important; }
[data-baseweb="checkbox"] input:checked+div { background:#ff72b6!important; border-color:#ff72b6!important; }
[data-testid="stProgress"] > div > div > div > div { background-color:#ff72b6!important; }
[data-testid="stProgress"] [role="progressbar"] > div { background-color:#ff72b6!important; }
[data-baseweb="progress-bar"] > div { background-color:#ff72b6!important; }
[data-baseweb="switch"] input:checked + div, [data-baseweb="switch"] [aria-checked="true"] { background-color:#ff72b6!important; border-color:#ff72b6!important; }
[data-testid="stFileUploaderDropzone"] button { color:var(--ink)!important; border-color:var(--ink)!important; }
[data-testid="stFileUploaderDropzone"] button:hover { background:#ffd9ec!important; color:var(--ink)!important; }
a:focus-visible, button:focus-visible, input:focus-visible, textarea:focus-visible, select:focus-visible { outline-color:#ff72b6!important; }
[data-baseweb="radio"] input:checked+div { background:#ff72b6!important; border-color:#ff72b6!important; }
[data-testid="stWidgetLabel"] p,[data-testid="stSliderTickBarMin"],[data-testid="stSliderTickBarMax"] { color:#7a4762!important; }
[data-baseweb="menu"],[data-baseweb="popover"] { background:#fff7fc!important; color:#4a2740!important; }
[data-baseweb="base-input"], [data-baseweb="textarea"] textarea { background:#fafcfd!important; color:#4a2740!important; caret-color:#ff72b6!important; }
[data-baseweb="select"] [role="combobox"], [role="listbox"], [role="option"] { background:#fffafd!important; color:#4a2740!important; }
[role="option"]:hover,[role="option"][aria-selected="true"] { background:#ffd9ec!important; color:#4a2740!important; }
[role="tooltip"], #vg-tooltip-element { background:#fff7fc!important; color:#4a2740!important; border:1px solid #e7a7c7!important; }
[data-testid="stMetricDelta"] { color:#7a4762!important; background:#ffe3f1!important; }
[data-testid="stMetricDelta"] svg { fill:currentColor!important; }
[data-testid="stAlert"] p,[data-testid="stAlert"] strong { color:#4a2740!important; }
[data-testid="stMarkdownContainer"] a { color:#ff5ca8!important; }
[data-testid="stMarkdownContainer"] code { color:#7a4762!important; background:#ffe3f1!important; }
::selection { background:#ffd4e8; color:#4a2740; }
...
def render_price_chart(frame):
    ...
    palette = ["#ff72b6", "#c995ff", "#ffaacf", "#ff9fe0"]
    chart = (
        alt.Chart(data)
        ...
        .configure(background="#fffafd", font="Arial")
        .configure_view(strokeOpacity=0)
        .configure_axis(labelColor="#7a4762", titleColor="#4a2740", gridColor="#f3c9de",
                        domainColor="#e7a7c7", tickColor="#e7a7c7")
        .configure_legend(labelColor="#7a4762", titleColor="#4a2740")
        .interactive()
    )
...
with st.sidebar:
    st.markdown('<div class="sidebar-brand">Femboy Investing</div><div class="sidebar-description">Your personal kawaii research workspace.</div>', unsafe_allow_html=True)
